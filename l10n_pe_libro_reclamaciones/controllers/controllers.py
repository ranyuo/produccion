...
class LibroReclamaciones(http.Controller):
    ...
    @http.route('/reclamacion-enviada/<string:code>', type='http', auth='public', methods=['GET', 'POST'], website=True,)
    def LibroReclamacionesEnviada(self, code, **kw):
        days = request.env.company.sudo().default_claim_attention_period
        return request.render('l10n_pe_libro_reclamaciones.reclamacion_enviada', {"code": code,"days":days})
    ...
    @http.route('/libro-reclamaciones', type='http', auth='public', methods=['GET', 'POST'], website=True)
